...
캡슐화(Encapsulation)
	...
	로직을 하나로 묶는주는 것을 말한다.
*/
//아래 4가지 업무는 번호순서데로 진행해야 한다고 가정한다.
class MemberRegist {
	void doMemberRegist() {
...
public class E05Encapsulation {
	...
	public static void main(String[] args) {
		...
		/*
		 업무의 진행순서가 중요하다고 가정했을때 캡슐화 이전의 코드
		 에서는 개발자가 순서를 지키지 않은 상태에서도 실행이 가능한 
		 코드가 된다. 즉, 업무의 차질이 발생할 수 있다.
		 */
		MemberRegist memberRegist = new MemberRegist();
		FirstLoginEvent firstLoginEvent = new FirstLoginEvent();
		AutoLogin autoLogin = new AutoLogin();
		...
		System.out.println("===============================");
		// 아래 메서드 호출을 통해 업무를 순서에 맞게 진핼할 수 있다.
