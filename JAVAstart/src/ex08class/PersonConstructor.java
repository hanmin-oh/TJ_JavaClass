...
//	public PersonConstructor() {}
	...
	public PersonConstructor(String name) {	
		/*
		 ...
		 는데, 클래스 자신, 즉 멤버를 가리킨다. 따라서 좌측항은 멤버
		 변수 우측항은 매개변수를 가리키게된다.
		 */
		this.name = name; 
		age = 1;
		addr = "출처불명";
	...
	public PersonConstructor(String name, int age) {
		/*
		 ...
		 사용할 수 없다. 즉, 아래는 코드는 매개변수가 3개인 생성자를 
		 호출하게 된다.
		 해당 코드는 생성자 내에서만 사용할 수 있다.
		 생성자는 객체 생성시 딱 한번만 호출되고 개발자가 임의로 호출할 
		 수 없다. 		
		 */
		this(name, age, "미상");
	...
	void initialize(String name, int age, String addr) {
		/*
		 멤버메서드는 개발자가 원할때 언제든 호출이 가능하므로 
		 생성자와는 용도와 특성이 다르다. 따라서 멤버메서드에서는 생성
		 자를 호출할 수 없다.
		 */	
		// 일반함수에서는 사용할 수 없다
