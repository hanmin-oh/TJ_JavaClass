...
class MyStatic {
	 ...
	 */
	void instanceMethod() {
		System.out.println("instanceVar="+ instanceVar); // 인스턴스형
		System.out.println("staticVar="+ staticVar); // static형
		staticMethod(); //메서드영역에 저장
	}
	/*
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		static(정적) 멤버변수 접근 방법 : 클래스명을 통해 직접 접근한다. 
			객체를 생성하지 않고 즉시 접근할 수 있다.
		 */
		MyStatic.staticVar = 200;   // new 가 없음.
		...
		없다.)
		 */
		myStatic.staticVar = 300;   // 이렇게 쓰지마 바보들아
		System.out.println("myStatic.staticVar="+ myStatic.staticVar);
		...
		/*
		클래스명을 통한 접근은 정적형 메서드만 가능하다. 인스턴스형 
		메서드는 객체의 참조변수를 통해서만 접근할 수 있다.
		 */
//		MyStatic.instanceMethod();  // 에러발생
		/*
		정적메서드는 아래와 같이 클래스명을 통해 접근하는 것을 
	    권장한다.
		 */
		MyStatic.staticMethod();  // 권장사항
		/*
		결론 :
		일번적인 인스턴스형 멤버는 객체의 참조변수를 통해 접근한다.
