...
public class TextFileWriteTest {
	...
	public static void main(String[] args) {
		...
		Scanner scanner = null; //키보드로 입력에 사용할 스캐너
		PrintWriter printWriter = null; //텍스트 파일로 출력에 사용할
		...
		"/"로 수정한다.
		-C:\ : 최상위경로 ":"은 드라이브를 뜻함 ,,, C: -- C드라이브
		*/
		/*절대경로
		String filepath = "C:\\k-digital\\java\\workspace\\k20230419\\src\\com\\tjoeun\\textFileIO\\out.txt";
		String filepath = "C:/k-digital\\java\\workspace/k20230419\\src\\com\\tjoeun\\textFileIO/put.txt";
		*/
		//상대경로
		String filepath = ".\\src\\com\\tjoeun\\textFileIO/out.txt";
		
		//PrintWriter 클래스의 생정자로 출력 파일의 경로와 이름을 지정한다.
		try {
			//PrintWriter 클래스의 생성자로 출력 파일의 경로와 이름을 지정해서 출력 파일을 만든다.
			printWriter = new PrintWriter(filepath);
			scanner = new Scanner(System.in);
			
			//"QUIT"가 입력될 때가지 반복하며 키보드로 입력한 데이터를 파일에 저장한다.
			while (true) {
				//텍스트 파일에 저장할 데이터를 키보드로 입력
				System.out.println(">>> ");
				...
				- write()메소드로 메소드로 출력할 때 줄을 바꾸기 위해 "\n"을 입력했음에도 불구하고 텍스트 파일에서 
				줄이 변경되서 보이지 않는다면 "\r\n"을 사용한다. => 순서 바꾸면 안된다.
