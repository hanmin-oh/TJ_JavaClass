...
public class TextFileReadTest2 {
	...
	public static void main(String[] args) {
		...
		String filepath = ".\\src\\com\\tjoeun\\textFileIO/input.txt";
		...
		try {
			...
			while(scanner.hasNextLine()) {
				...
				//scanner 클래스의 객체를 만들 때 생성자의 인수로 String 변수를 넘기면 Srting 변수에
				//저장된 데이터를 읽어들인다.
				Scanner scan = new Scanner(str); //문자열에서 데이터를 읽어들이는 스캐너
				/*hasNext() 메소드는 스캐너로 지정된 문자열에서 공백을 경계로 읽어들일 데이터가 있으면
				true, 없으면 false 를 리턴한다.*/
				while (scan.hasNext()) {
